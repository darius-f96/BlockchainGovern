...
class CountyController {
    ...
    @Autowired
    CountyDAO repository;

    @GetMapping
    public ResponseEntity<List<County>> getAll() {
        try {
            List<County> items = new ArrayList<County>();
    ...
    @PostMapping
    public ResponseEntity<County> create(@RequestBody County item) {
        try {
            County savedItem = repository.save(item);
