...
class CompanyAddressController {
    ...
    @Autowired
    CompanyAddressDAO repository;

    @GetMapping
    public ResponseEntity<List<CompanyAddress>> getAll() {
        try {
            List<CompanyAddress> items = new ArrayList<CompanyAddress>();
    ...
    @PostMapping
    public ResponseEntity<CompanyAddress> create(@RequestBody CompanyAddress item) {
        try {
            CompanyAddress savedItem = repository.save(item);
